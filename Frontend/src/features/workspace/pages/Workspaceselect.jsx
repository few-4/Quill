            ...
            {isLoading ? (
              Array.from({ length: 2 }).map((_, i) => (
                <div key={i} className="flex items-start gap-4 p-5 rounded-2xl bg-theme-card/40 border border-theme-border/50 animate-pulse">
                  <div className="w-12 h-12 rounded-xl bg-theme-border/60 flex-shrink-0" />
              ...
              workspaces.map((ws) => {
                ...
                return (
                  <div
                    ...
                    className="group relative flex items-start gap-4 p-5 rounded-2xl bg-theme-card/65 border border-theme-border/80 backdrop-blur-md cursor-pointer hover:border-theme-txt-secondary/30 hover:bg-theme-card/90 hover:-translate-y-[2px] transition-all duration-300 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] select-none animate-in fade-in-50 slide-in-from-bottom-2 duration-300"
                  >
                    {/* Glowing dynamic background highlight on hover */}
                    <div className="absolute inset-0 rounded-2xl bg-gradient-to-tr from-brand-blue/0 to-brand-pink/0 group-hover:from-brand-blue/[0.02] group-hover:to-brand-pink/[0.02] transition-colors duration-300 pointer-events-none" />

                    {/* Icon Avatar */}
                    <div className={`w-12 h-12 rounded-xl flex-shrink-0 bg-gradient-to-tr ${bgGradient} flex items-center justify-center font-bold text-lg shadow-inner shadow-black/10`}>
